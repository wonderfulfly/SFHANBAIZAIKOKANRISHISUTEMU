...
function afterSave() {
    ...
    if (getObj('WF_RUNRESULT').value =='1') {
        Grid19_clear();
getObj('initRowGrid19').value = getObj('RowGrid19').value
...
function resetDropdownListOptionStyle(targetObj) {
  ...
  /* filter end */
  function PageDefaultSet(){var CHUUBUNSHONO1;
var CHUUBUNSHOMEISAINO;
var KOUJIKANRIBANGOU;
var ZAIKONO1;
var TSUKAMATSUNYUUSAKI;
var SHOUHINMEISHOU;
var SHUTSUNYUUKOJIKAN;
var SUURYOU;
var PRICE;
var SHUTSUNYUUKOJOUTAI;
var grdB8AE8;
var SHUKKANO;
var SHOUHINKATABAN;
var ZAIKOSUURYOU;
...
/*Grid「B8」ループ開始。*/
var intRow = getObj("RowGrid19").value;

for (var i = 1;i <= intRow; i++) {
/* grid loop start */
	CHUUBUNSHONO1 = $('#CHUUBUNSHONO1_'+i).val();
	CHUUBUNSHOMEISAINO = $('#CHUUBUNSHOMEISAINO_'+i).val();
	KOUJIKANRIBANGOU = $('#KOUJIKANRIBANGOU_'+i).val();
	ZAIKONO1 = $('#ZAIKONO1_'+i).val();
	TSUKAMATSUNYUUSAKI = $('#TSUKAMATSUNYUUSAKI_'+i).val();
	SHOUHINMEISHOU = $('#SHOUHINMEISHOU_'+i).val();
	SHUTSUNYUUKOJIKAN = $('#SHUTSUNYUUKOJIKAN_'+i).val();
	SUURYOU = $('#SUURYOU_'+i).val();
	PRICE = $('#PRICE_'+i).val();
	SHUTSUNYUUKOJOUTAI = $('#SHUTSUNYUUKOJOUTAI_'+i).val();
	grdB8AE8 = $('#grdB8AE8_'+i).val();
	SHUKKANO = $('#SHUKKANO_'+i).val();
	SHOUHINKATABAN = $('#SHOUHINKATABAN_'+i).val();
	ZAIKOSUURYOU = $('#ZAIKOSUURYOU_'+i).val();
	SHOUHINNO = $('#SHOUHINNO_'+i).val();
	/*1.「状態」が"1"又は「状態」が"2"の場合,以下の処理を行う。*/
	if (SHUTSUNYUUKOJOUTAI == "1" || SHUTSUNYUUKOJOUTAI == "2") {
		/*1．「状態」が"1"の場合,以下の処理を行う。*/
		if (SHUTSUNYUUKOJOUTAI == "1") {
			/*1．「入庫」ボタンに"キャンセル"を設定する。*/
			$('#grdB8AE8_'+i).val("キャンセル");
			$('#grdB8AE8_'+i).blur();
			/*処理終了。*/
		}
		/*2.その他の場合,以下の処理を行う。*/ else {
			/*1.項目「入庫」を非表示にする。*/
			SF.setVisible('grdB8AE8'+"_"+i, false);
			/*処理終了。*/
		}
		/*処理終了。*/
	}
...
if (getObj("Grid19flg" + i) &&  (getObj("Grid19flg" + i).value != '')) {
 		if(!isLenCheck(getObj('SHUKKANO_' + i), 20, "出荷NO", true)) { alertMsg = '' + i +'行目の[出荷NO]項目に入力できるのは[最大長さは20の値]のみです。'; alert(alertMsg); return false; } 		if(!isLenCheck(getObj('SHOUHINKATABAN_' + i), 20, "商品型番", true)) { alertMsg = '' + i +'行目の[商品型番]項目に入力できるのは[最大長さは20の値]のみです。'; alert(alertMsg); return false; } 		if(!checkIntegerType(getObj('ZAIKOSUURYOU_' + i), "在庫数量", true, "項目「@@」には数値を入力して下さい。")) { alertMsg = '' + i +'行目の[在庫数量]項目に入力できるのは[数字+[-]]のみです。'; alert(alertMsg); return false; } 		if(!isLenCheck(getObj('SHOUHINNO_' + i), 15, "商品NO", true)) { alertMsg = '' + i +'行目の[商品NO]項目に入力できるのは[最大長さは15の値]のみです。'; alert(alertMsg); return false; }}	}
...
${PageBucket.knockOutScriptMap.get("dataChange")}
  ...
  var Grid19;
 function setGrid19(recordsPerPage) {
var recordNum = 10;
if (recordsPerPage != undefined) {
 recordNum = recordsPerPage;
}
Grid19 = $("#Grid19").aingrid({
 	height:199,
 	width:919,
 	displayWidth:919,
 	isMobile:false,
 	initialLoad: false,
 	colWidths: [50,91,94,82,86,84,102,89,55,55,56,74,0,0,0,0],
 	...
 	colDataId: ['lvl','CHUUBUNSHONO1_','CHUUBUNSHOMEISAINO_','KOUJIKANRIBANGOU_','ZAIKONO1_','TSUKAMATSUNYUUSAKI_','SHOUHINMEISHOU_','SHUTSUNYUUKOJIKAN_','SUURYOU_','PRICE_','SHUTSUNYUUKOJOUTAI_','grdB8AE8_','SHUKKANO_','SHOUHINKATABAN_','ZAIKOSUURYOU_','SHOUHINNO_'],
 	autoRowId: false,
  colsetting: false,
 	rowActionFlg: 'Grid19flg',
 	rowDelChkFlg: 'Grid19delchk',
 	rowBegin: 1,
 	...
 	totalRecords: parseInt($("#initRowGrid19").val()),
 	scrollbarW: 17,
 	sorting: false,
 	sortedCol:0,
 	sortedColDir:"asc",
 	...
 	useTemplateRow: true,
 	actionFlg: 2,
 	onAfterPaging: function(){ ajaxDoAddExeJs("A007grdB81Change",null, null, $(this));},
 	allRowSent: 1,
 	rowAdd: false,
 	rowDelete:false,
 	rowUpdate:true,
 ...
 effectDisplay:0
 	...
 	$('#_ingrid_Grid19_0_b .autocomp').each(function () {
		var row = $(this).parents('tr').index() + 1;
	
	});
 SF.addGridHeadListener(19, Grid19);
}
...
<div id="dragB1"  style="position:absolute;top:24px;left:220px;text-align:right;padding-right:2px;vertical-align:bottom;width:90px;height:32px;z-index:1;" class='userDiv' _ckf='0'><label for='CHUUBUNSHONO'  style='display:table-cell;width:90px;height:32px;line-height:32px;text-align:right;padding-right:2px;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;' _subid=1 class='' id="I2"><b>注文書No</b></label></div>
<div id="dragB3"  style="position:absolute;top:24px;left:312px;text-align:left;width:146px;height:34px;z-index:1;" class='userDiv' _ckf='0'><input id='CHUUBUNSHONO'  name='CHUUBUNSHONO'  value='${requestScope.CHUUBUNSHONO}'  class='editable' style='width:146px;height:34px;padding:0 2px;text-align:left;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;'  _subid='3'  _itemname='注文書No'  type='text'  tabindex='2'></div>
<div id="dragB5"  style="position:absolute;top:24px;left:538px;text-align:right;padding-right:2px;vertical-align:bottom;width:71px;height:32px;z-index:1;" class='userDiv' _ckf='0'><label for='BUHINMEI'  style='display:table-cell;width:71px;height:32px;line-height:32px;text-align:right;padding-right:2px;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;' _subid=5 class='' id="U2"><b>部品名</b></label></div>
<div id="dragB7"  style="position:absolute;top:24px;left:611px;text-align:left;width:167px;height:34px;z-index:1;" class='userDiv' _ckf='0'><input id='BUHINMEI'  name='BUHINMEI'  value='${requestScope.BUHINMEI}'  class='editable' style='width:167px;height:34px;padding:0 2px;text-align:left;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;'  _subid='7'  _itemname='部品名'  type='text'  tabindex='4'></div>
<div id="dragB9"  style="position:absolute;top:82px;left:235px;text-align:right;padding-right:2px;vertical-align:bottom;width:75px;height:32px;z-index:1;" class='userDiv' _ckf='0'><label for='ZAIKONO'  style='display:table-cell;width:75px;height:32px;line-height:32px;text-align:right;padding-right:2px;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;' _subid=9 class='' id="I4"><b>在庫NO</b></label></div>
<div id="dragB11"  style="position:absolute;top:82px;left:312px;text-align:left;width:146px;height:34px;z-index:1;" class='userDiv' _ckf='0'><input id='ZAIKONO'  name='ZAIKONO'  value='${requestScope.ZAIKONO}'  class='editable' style='width:146px;height:34px;padding:0 2px;text-align:left;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;'  _subid='11'  _itemname='在庫NO'  type='text'  tabindex='6'></div>
<div id="dragB13"  style="position:absolute;top:82px;left:504px;text-align:right;padding-right:2px;vertical-align:bottom;width:105px;height:32px;z-index:1;" class='userDiv' _ckf='0'><label for='SHANAIKOUJIBANGOU'  style='display:table-cell;width:105px;height:32px;line-height:32px;text-align:right;padding-right:2px;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;' _subid=13 class='' id="U4"><b>社内工事番号</b></label></div>
<div id="dragB15"  style="position:absolute;top:82px;left:611px;text-align:left;width:167px;height:34px;z-index:1;" class='userDiv' _ckf='0'><input id='SHANAIKOUJIBANGOU'  name='SHANAIKOUJIBANGOU'  value='${requestScope.SHANAIKOUJIBANGOU}'  class='editable' style='width:167px;height:34px;padding:0 2px;text-align:left;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;'  _subid='15'  _itemname='社内工事番号'  type='text'  tabindex='8'></div>
<div id="dragB17"  style="position:absolute;top:140px;left:670px;text-align:left;width:109px;height:32px;z-index:1;" class='userDiv' _ckf='0'><input id='Y6'  name='Y6'  value='検索'  style='width:109px;height:32px;padding:0 2px;text-align:center;vertical-align:middle;font-size:14px;font-weight:normal;' type='button'  onClick ='if (SF.isServerExecute()) {SF.setServerExecute(false);ajaxDoAddExeJs("A007Y61Click",null,null,$(this));}'  tabindex='9' class='button_blue' _sf_class='button_blue'></div>
<div id="dragB19"  style="position:absolute;top:196px;left:44px;text-align:left;height:199px;z-index:1;" class='userDiv' _ckf='0'><input type='hidden' name='initRowGrid19' id='initRowGrid19' value='0'><input type='hidden' name='RowGrid19' id='RowGrid19' value='0'><table id='Grid19' name ='Grid19' ><thead><tr><th style='text-align:center;'>No.</th><th style='overflow:hidden;text-align:center;'>注文書NO*</th><th style='overflow:hidden;text-align:center;'>注文書明細NO</th><th style='overflow:hidden;text-align:center;'>工事管理番号</th><th style='overflow:hidden;text-align:center;'>部品明細連番</th><th style='overflow:hidden;text-align:center;'>仕入先</th><th style='overflow:hidden;text-align:center;'>部品名称</th><th style='overflow:hidden;text-align:center;'>出入庫時間</th><th style='overflow:hidden;text-align:center;'>数量</th><th style='overflow:hidden;text-align:center;'>単価</th><th style='overflow:hidden;text-align:center;'>状態</th><th style='overflow:hidden;text-align:center;'>入庫</th><th style='display:none'>出荷NO</th><th style='display:none'>商品型番</th><th style='display:none'>在庫数量</th><th style='display:none'>商品NO</th></tr></thead>
<tbody>
<tr style='display:none' id ='rowseq0'><td align='right' class='g3 l' _colid=0>0<input type=hidden width=0px id='Grid19flg0' name='Grid19flg0'></td>
<td class='g1 o' _colid=1 style ='vertical-align:middle;'><div style="overflow:hidden;"><input id='CHUUBUNSHONO1_0'  name='CHUUBUNSHONO1_0'  class='readonly' readOnly style='width:88px;height:31px; text-align:left;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;'  _subid='21'  _itemname='注文書NO' onblur='changeSpecHtml(this);' type='text'  _gid='Grid19'  _gflg='Grid19flg0' ></div></td>
<td class='g1 o' _colid=2 style ='vertical-align:middle;'><div style="overflow:hidden;"><input id='CHUUBUNSHOMEISAINO_0'  name='CHUUBUNSHOMEISAINO_0'  class='readonly' readOnly style='width:91px;height:31px; text-align:left;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;'  _subid='23'  _itemname='注文書明細NO' onblur='changeSpecHtml(this);' type='text'  _gid='Grid19'  _gflg='Grid19flg0' ></div></td>
<td class='g1 o' _colid=3 style ='vertical-align:middle;'><div style="overflow:hidden;"><input id='KOUJIKANRIBANGOU_0'  name='KOUJIKANRIBANGOU_0'  class='readonly' readOnly style='width:79px;height:31px; text-align:left;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;'  _subid='25'  _itemname='工事管理番号' onblur='changeSpecHtml(this);' type='text'  _gid='Grid19'  _gflg='Grid19flg0' ></div></td>
<td class='g1 o' _colid=4 style ='vertical-align:middle;'><div style="overflow:hidden;"><input id='ZAIKONO1_0'  name='ZAIKONO1_0'  value='[自動採番]'  _k='1'  class='readonly' readOnly style='width:81px;height:25px;text-align:left;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;'  _subid='27'  _itemname='部品明細連番' onblur='changeSpecHtml(this);' type='text'  _gid='Grid19'  _gflg='Grid19flg0' ></div></td>
<td class='g1 l' _colid=5 style='word-wrap: break-word;'><input type=text name=showTSUKAMATSUNYUUSAKI_0 id=showTSUKAMATSUNYUUSAKI_0 value='WFC#ITEM_SHOW_VALUE#WFC29WFC#ITEM_SHOW_VALUE#WFC' class='readonly' readOnly _subid=29 _itemname='仕入先' style='width:83px;height:27px;vertical-align:middle;display: table-cell;vertical-align: middle;font-size:14px;color:#000000;font-weight:normal;'><input type=hidden name=TSUKAMATSUNYUUSAKI_0 id=TSUKAMATSUNYUUSAKI_0 value='WFC#COLUMN_VALUE#WFC29WFC#COLUMN_VALUE#WFC'></td>
<td class='g1 o' _colid=6 style ='vertical-align:middle;'><div style="overflow:hidden;"><input id='SHOUHINMEISHOU_0'  name='SHOUHINMEISHOU_0'  class='readonly' readOnly style='width:99px;height:31px; text-align:left;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;'  _subid='31'  _itemname='部品名称' onblur='changeSpecHtml(this);' type='text'  _gid='Grid19'  _gflg='Grid19flg0' ></div></td>
<td class='g2 o' _colid=7 style ='vertical-align:middle;'><div style="overflow:hidden;"><input type=text name=SHUTSUNYUUKOJIKAN_0 id=SHUTSUNYUUKOJIKAN_0 data-bind='value: SHUTSUNYUUKOJIKAN' value='' class='readonly' readOnly _subid=33 _itemname='出入庫時間' _gid='Grid19' _gflg='Grid19flg0' onChange ='getObj("SHUTSUNYUUKOJIKAN_0").value=this.value;dateFormatChange($(this),"出入庫時間");cs($(this),"rowseq","Grid19flg","0");'  style='width:86px;height:29px; padding:0 2px;text-align:center;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;ime-mode:disabled;z-index:1;' placeHolder=''></div></td>
<td class='g3 o' _colid=8 style ='vertical-align:middle;'><div style="overflow:hidden;"><input id='SUURYOU_0'  name='SUURYOU_0'  class='readonly' readOnly style='width:52px;height:31px; text-align:right;padding-right:2px;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;ime-mode:disabled;'  _subid='35'  _itemname='数量'  _rge='2' onblur='changeSpecHtml(this);' type='text'  _gid='Grid19'  _gflg='Grid19flg0' ></div></td>
<td class='g3 o' _colid=9 style ='vertical-align:middle;'><div style="overflow:hidden;"><input id='PRICE_0'  name='PRICE_0'  class='readonly' readOnly style='width:52px;height:31px; text-align:right;padding-right:2px;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;ime-mode:disabled;'  _subid='37'  _itemname='単価'  _ire='###,##0.##'  _rge='1' onblur='changeSpecHtml(this);' type='text'  _gid='Grid19'  _gflg='Grid19flg0' ></div></td>
<td class='g1 l' _colid=10 style='word-wrap: break-word;'><input type=text name=showSHUTSUNYUUKOJOUTAI_0 id=showSHUTSUNYUUKOJOUTAI_0 value='WFC#ITEM_SHOW_VALUE#WFC39WFC#ITEM_SHOW_VALUE#WFC' class='readonly' readOnly _subid=39 _itemname='状態' style='width:55px;height:27px;vertical-align:middle;display: table-cell;vertical-align: middle;font-size:14px;color:#000000;font-weight:normal;'><input type=hidden name=SHUTSUNYUUKOJOUTAI_0 id=SHUTSUNYUUKOJOUTAI_0 value='WFC#COLUMN_VALUE#WFC39WFC#COLUMN_VALUE#WFC'></td>
<td class='g1 o' _colid=11><div style="overflow:hidden;"><input id='grdB8AE8_0'  name='grdB8AE8_0'  value='入庫'  type='button'  onClick ='if (SF.isServerExecute()) {SF.setServerExecute(false);getObj("grdB8AE8_0").value=this.value;ajaxDoAddExeJs("A007grdB8AE81Click",null,null,$(this));}'  _gid='Grid19'  _gflg='Grid19flg0'  class='button_red' _sf_class='button_red'></div></td>
<td style ='display:none' width='0px' _colid=12><input id='SHUKKANO_0'  name='SHUKKANO_0'  type='hidden' ></div></td>
<td style ='display:none' width='0px' _colid=13><input id='SHOUHINKATABAN_0'  name='SHOUHINKATABAN_0'  type='hidden' ></div></td>
<td style ='display:none' width='0px' _colid=14><input id='ZAIKOSUURYOU_0'  name='ZAIKOSUURYOU_0'  type='hidden' ></div></td>
<td style ='display:none' width='0px' _colid=15><input id='SHOUHINNO_0'  name='SHOUHINNO_0'  type='hidden' ></div></td></tr></tbody></table></div>
   </div>
...
getQAjaxRunJson("rstfld",false,1,"","A007Init.do",true,setSendData(document.form1),"");
