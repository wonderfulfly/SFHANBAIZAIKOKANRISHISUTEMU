...
function resetDropdownListOptionStyle(targetObj) {
  ...
  function printG4Check(){

 return true;
}
function printL4Check(){
...
if (getObj("Grid19flg" + i) &&  (getObj("Grid19flg" + i).value != '')) {
 		if(!isLenCheck(getObj('SHOUHINMEISHOU_' + i), 100, "商品名称", true)) { alertMsg = '' + i +'行目の[商品名称]項目に入力できるのは[最大長さは100の値]のみです。'; alert(alertMsg); return false; } 		if(!isLenCheck(getObj('ZAISHITSU_' + i), 20, "材質", true)) { alertMsg = '' + i +'行目の[材質]項目に入力できるのは[最大長さは20の値]のみです。'; alert(alertMsg); return false; } 		if(!isDate(getObj('NYUUKAHIDZUKE_' + i), false, "入荷日付", true)) { alertMsg = '' + i +'行目の[入荷日付]項目に入力できるのは[日付]のみです。'; alert(alertMsg); return false; } 		if(!isNumericType(getObj('PRICE_' + i), "単価", true)) { return false;}
	 if(!isInteger(getObj('PRICE_' + i),false,"単価",10,true,true)) { alertMsg = '' + i +'行目の[単価]項目に入力できるのは[数字のみ]のみです。'; alert(alertMsg); return false; } 		if(!checkIntegerType(getObj('ZAIKOSUURYOU_' + i), "購買数量", true, "項目「@@」には数値を入力して下さい。")) { alertMsg = '' + i +'行目の[購買数量]項目に入力できるのは[数字+[-]]のみです。'; alert(alertMsg); return false; }
	 if(!isInteger(getObj('ZAIKOSUURYOU_' + i),false,"購買数量",10,true,true)) { alertMsg = '' + i +'行目の[購買数量]項目に入力できるのは[数字のみ]のみです。'; alert(alertMsg); return false; }}	}
...
Grid19 = $("#Grid19").aingrid({
 	...
 	isMobile:false,
 	initialLoad: false,
 	colWidths: [50,185,83,197,117,86,82,0],
 	rowClasses: ['ui-grid-row-style1', 'ui-grid-row-style2'],
 	addRowMethod: 2,
 	bodyHeight: 34,
 	hasFooter: true,
 	hasScroll: true,
 	colDataType: [0,2,2,2,2,2,2,2],
 	colDataId: ['lvl','SHOUHINMEISHOU_','ZAISHITSU_','TSUKAMATSUNYUUSAKI_','NYUUKAHIDZUKE_','PRICE_','ZAIKOSUURYOU_','SHOUHINNO_'],
 	autoRowId: false,
  colsetting: false,
 	...
 	colClasses:['grid-col-num','g1','g1','g1','g2','g3','g3','g1'],
 	resizableCols: false,
 	url: 'A006GridFormFeed.do?pageID=P00008&subID=19',
 	extraParams: {},
 	paging: true,
...
<li><span>商品管理</span></li>
	<li class="current"><em>商品(P00008 - A006)</em></li>
</ul>
</div>
...
<div id="dragB1"  style="position:absolute;top:24px;left:127px;text-align:right;padding-right:2px;vertical-align:bottom;width:60px;height:32px;z-index:1;" class='userDiv' _ckf='0'><label for='SHOUHINMEI'  style='display:table-cell;width:60px;height:32px;line-height:32px;text-align:right;padding-right:2px;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;' _subid=1 class='' id="D2"><b>商品名</b></label></div>
<div id="dragB3"  style="position:absolute;top:24px;left:189px;text-align:left;width:168px;height:34px;z-index:1;" class='userDiv' _ckf='0'><input id='SHOUHINMEI'  name='SHOUHINMEI'  value='${requestScope.SHOUHINMEI}'  class='editable' style='width:168px;height:34px;padding:0 2px;text-align:left;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;'  _subid='3'  _itemname='商品名'  type='text'  tabindex='2'></div>
<div id="dragB5"  style="position:absolute;top:24px;left:434px;text-align:right;padding-right:2px;vertical-align:bottom;width:60px;height:32px;z-index:1;" class='userDiv' _ckf='0'><label for='KAISHAMEI'  style='display:table-cell;width:60px;height:32px;line-height:32px;text-align:right;padding-right:2px;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;' _subid=5 class='' id="O2"><b>会社名</b></label></div>
<div id="dragB7"  style="position:absolute;top:24px;left:496px;text-align:left;width:201px;height:34px;z-index:1;" class='userDiv' _ckf='0'><select id=KAISHAMEI name=KAISHAMEI _req=0 class='editable chosen_dropdownlist_style ' _subid=7 _itemname='会社名' style='width:201px;height:32px;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;' tabindex='4'><option value ='${requestScope.KAISHAMEI}' selected>${requestScope.KAISHAMEI}</option></select></div>
<div id="dragB9"  style="position:absolute;top:24px;left:903px;text-align:left;display:none;width:91px;height:34px;z-index:1;" class='userDiv' _ckf='0'><input id='AD2'  name='AD2'  value='保存'  style='width:91px;height:34px;padding:0 2px;padding:0 2px;text-align:center;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;display:none;' type='button'  onClick ='if (SF.isServerExecute()) {SF.setServerExecute(false);getObj("AD2").value=this.value;F20091208153604208();}'  tabindex='5' class='btn02' _sf_class='btn02'></div>
<div id="dragB11"  style="position:absolute;top:83px;left:189px;text-align:left;width:139px;height:32px;z-index:1;" class='userDiv' _ckf='0'><input id='G4'  name='G4'  value='商品一覧'  style='width:139px;height:32px;padding:0 2px;text-align:center;vertical-align:middle;font-size:14px;font-weight:normal;' type='button'  onClick ='if (SF.isServerExecute()) {SF.setServerExecute(false); if (printG4Check()) if(confirm("ファイルを出力してもよろしいですか？")) {ajaxDoExcelPrint("A006","G4");} else {SF.setServerExecute(true);}}'  tabindex='6' class='btn04' _sf_class='btn04'></div>
<div id="dragB13"  style="position:absolute;top:83px;left:358px;text-align:left;width:167px;height:32px;z-index:1;" class='userDiv' _ckf='0'><input id='L4'  name='L4'  value='入庫出庫情報一覧'  style='width:167px;height:32px;padding:0 2px;text-align:center;vertical-align:middle;font-size:14px;font-weight:normal;' type='button'  onClick ='if (SF.isServerExecute()) {SF.setServerExecute(false); if (printL4Check()) if(confirm("ファイルを出力してもよろしいですか？")) {ajaxDoExcelPrint("A006","L4");} else {SF.setServerExecute(true);}}'  tabindex='7' class='btn04' _sf_class='btn04'></div>
<div id="dragB15"  style="position:absolute;top:83px;left:564px;text-align:left;width:133px;height:32px;z-index:1;" class='userDiv' _ckf='0'><input id='S4'  name='S4'  value='検索'  style='width:133px;height:32px;padding:0 2px;text-align:center;vertical-align:middle;font-size:14px;font-weight:normal;' type='button'  onClick ='if (SF.isServerExecute()) {SF.setServerExecute(false); doSmartSearch("S4","19;grdB6","1");}'  tabindex='8' class='button_blue' _sf_class='button_blue'></div>
<div id="dragB17"  style="position:absolute;top:83px;left:903px;text-align:left;display:none;width:91px;height:32px;z-index:1;" class='userDiv' _ckf='0'><input id='AD4'  name='AD4'  value='商品インポート'  style='width:91px;height:32px;padding:0 2px;padding:0 2px;text-align:center;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;display:none;' type='button'  tabindex='9' class='btn04' _sf_class='btn04'></div>
<div id="dragB19"  style="position:absolute;top:139px;left:61px;text-align:left;height:141px;z-index:1;" class='userDiv' _ckf='0'><input type='hidden' name='initRowGrid19' id='initRowGrid19' value='0'><input type='hidden' name='RowGrid19' id='RowGrid19' value='0'><table id='Grid19' name ='Grid19' ><thead><tr><th style='text-align:center;'>No.</th><th style='overflow:hidden;text-align:center;'>商品名称</th><th style='overflow:hidden;text-align:center;'>材質</th><th style='overflow:hidden;text-align:center;'>仕入先</th><th style='overflow:hidden;text-align:center;'>入荷日付</th><th style='overflow:hidden;text-align:center;'>単価</th><th style='overflow:hidden;text-align:center;'>購買数量</th><th style='display:none'>商品NO</th></tr></thead>
<tbody>
...
<td class='g1 o' _colid=1 style ='vertical-align:middle;'><div style="overflow:hidden;"><input id='SHOUHINMEISHOU_0'  name='SHOUHINMEISHOU_0'  class='editable' style='width:182px;height:31px; padding:0 2px;text-align:left;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;'  _subid='21'  _itemname='商品名称'  type='text'  _gid='Grid19'  _gflg='Grid19flg0' ></div></td>
<td class='g1 o' _colid=2 style ='vertical-align:middle;'><div style="overflow:hidden;"><input id='ZAISHITSU_0'  name='ZAISHITSU_0'  class='editable' style='width:80px;height:31px; padding:0 2px;text-align:left;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;'  _subid='23'  _itemname='材質'  type='text'  _gid='Grid19'  _gflg='Grid19flg0' ></div></td>
<td class='g1 l' _colid=3 style='word-wrap: break-word;'><input type=text name=showTSUKAMATSUNYUUSAKI_0 id=showTSUKAMATSUNYUUSAKI_0 value='WFC#ITEM_SHOW_VALUE#WFC25WFC#ITEM_SHOW_VALUE#WFC' class='readonly' readOnly _subid=25 _itemname='仕入先' style='width:196px;height:27px;vertical-align:middle;display: table-cell;vertical-align: middle;font-size:14px;color:#000000;font-weight:normal;'><input type=hidden name=TSUKAMATSUNYUUSAKI_0 id=TSUKAMATSUNYUUSAKI_0 value='WFC#COLUMN_VALUE#WFC25WFC#COLUMN_VALUE#WFC'></td>
<td class='g2 o' _colid=4 style ='vertical-align:middle;'><div style="overflow:hidden;"><input type=text name=NYUUKAHIDZUKE_0 id=NYUUKAHIDZUKE_0 data-bind='value: NYUUKAHIDZUKE' value='' class='editable calendar' _subid=27 _itemname='入荷日付' _gid='Grid19' _gflg='Grid19flg0' onChange ='getObj("NYUUKAHIDZUKE_0").value=this.value;dateFormatChange($(this),"入荷日付");cs($(this),"rowseq","Grid19flg","0");'  style='width:114px;height:29px; padding:0 2px;text-align:center;vertical-align:middle;font-size:14px;color:#000000;font-weight:normal;ime-mode:disabled;z-index:1;' placeHolder=''></div></td>
...
getQAjaxRunJson("rstfld",false,1,"","A006Init.do",true,setSendData(document.form1),"");
