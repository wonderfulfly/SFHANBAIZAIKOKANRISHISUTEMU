...
//		headerClass: 'grid-header-bg',			// header bg
		...
		gridClass: 'tbl_design01',					// class of head & body
...
//					.width(w)
	...
	var bt = jQuery('<table cellpadding="0" cellspacing="0" class="tbl_design01"></table>').html( this.find('tbody') ) ;
	var b = jQuery('<div />')
					.html( bt)
					.css('overflow', 'auto')
					.width(w);
...
//	.width(cfg.width)
	...
	g.extend({
		load : function(params, cb) {
			...
			var csd = getCookie("csd");
	
			if ($("#sortnum").val()==ss) {
				
				if (getObj("sortdir").value == "asc") {
					getObj("sortdir").value = "desc";
					jQuery.cookie(csd+"sortdir","desc");
				} else {
					getObj("sortdir").value = "asc";
					jQuery.cookie(csd+"sortdir","asc");
				}
			...
			getObj("sortnum").value=ss;
			
			jQuery.cookie(csd+"sortnum",ss);
			ajaxQuery(1,0);
			
//			jQuery.ajax({
//				type: cfg.type.toUpperCase(),
//				url: url,
//				data: setSendData(document.SearchFrm),
